...
# transformación logarítimica obtemos as variables do modelo (mod1):
#  Y   log(NewHIV)  resposta        continua
#  X1  log(PreHIV)  explicativa     continua
#  X2  log(TotHIV)  explicativa     continua
#  X3  log(Pop)     explicativa     continua

# Representación de datos

# Diagramas de dispersión da resposta sobre as explicativas (logarítmicas) (Disp3)
plotlog1 <- ggplot(data.frame(log(PreHIV),log(NewHIV)), aes(x = log(PreHIV), y = log(NewHIV))) +
  geom_point(color = "cadetblue1") + theme_minimal()
plotlog2 <- ggplot(data.frame(log(TotHIV),log(NewHIV)), aes(x = log(TotHIV), y = log(NewHIV))) +
  geom_point(color = "indianred1") + theme_minimal()
plotlog3 <- ggplot(data.frame(log(Pop),log(NewHIV)), aes(x = log(Pop), y = log(NewHIV))) +
  geom_point(color = "palegreen") + theme_minimal()
...
plotlog4 <- ggplot(data.frame(log(PreHIV),log(TotHIV)), aes(x = log(PreHIV), y = log(TotHIV))) +
  geom_point(color = "gold2") + theme_minimal()
plotlog5 <- ggplot(data.frame(log(Pop),log(TotHIV)), aes(x = log(Pop), y = log(TotHIV))) +
  geom_point(color = "orchid1") + theme_minimal()
plotlog6 <- ggplot(data.frame(log(PreHIV),log(Pop)), aes(x = log(PreHIV), y = log(Pop))) +
  geom_point(color = "gray") + theme_minimal()
...
# - No diagrama plotlog5 observamos unha relación directa, isto pode ser un problema 
#   de correlación.
...
rm(confint_df_99,confint_df_95,confint_df_90)
rm(confint_99,confint_95,confint_90)
...
# - Observamos que preto do (0,0) os datos alónxanse da banda de confianza para o QQplot
# - No histograma vemos como a distribución é picuda pero asimétrica.
...
corr_matrix <-round(cor(data_matrix),3)
...
# test realizados, rexeitar a hipóteses de normalidade, verificando homocedasticidade
# e independencia. Porén, hai problemas de colinealidade. Como
...
# -  Observamos que preto do (0,0) os datos alónxanse da banda de confianza para o QQplot
#    igual ca no modelo anterior, nótese que agora maís se alonxan da orixe.
...
# - O nivel de significación é alto e indícanos que o mellor modelo non é un modelo
#   de Regresión Lineal

# - Observamos que o estimador non paramétrico da regresión enmárcase dentro da rexión de
#   confianza, pero a meirande parte das observacións non están dentro da banda de confianza.
...
# - Os VIF melloran a situación do modelo anterior.
...
# - O p-valor é moi pequeno, logo rexeitamos a hipótese nula de que todas as medias son
...
#   a hipótese de normalidade dos residuos. O resto si que verifica

# Test de Shapiro-Wilk para os datos
resultado_shapiro <- shapiro.test(residuals(modAnovaReg))
print(resultado_shapiro) # 0.0641 

# - Observamos que a un nivel de 0.05 case podemos supor normalidade.
...
# Test de Shapiro-Wilk para os residuos por grupos
normalidad_residuos <- dataAnova %>%
  group_by(AnovaReg) %>%
  mutate(residuo = AnovaNew - mu_localReg[AnovaReg] ) %>%
  summarise(p_value = shapiro.test(residuo)$p.value, .groups = "drop")
print(normalidad_residuos)
...
ggplot(dataAncova, aes(x = AncovaPop, y = AncovaNew, colour = AncovaReg,shape=AncovaReg)) +
  ...
  labs(title = "Modelo ANCOVA sen Interacción", x = "AncovaPop", y = "AncovaNew",colour = "Región") +
  scale_colour_manual(values = c(
    "Americas" = "red",  
    "Eastern Mediterranean" = "purple",  
    "Europe" = "green",  
...
xAfr <- dataAncova[AncovaReg=="Africa",]$AncovaPop
modAfr <- lm(yAfr ~xAfr)
summary(modAfr)
plot(modAfr)
valAncova.df[1,2] <- shapiro.test(rstandard(modAfr))$p.value
valAncova.df[1,3] <- bptest(modAfr)$p.value
valAncova.df[1,4] <- hmctest(modAfr)$p.value
valAncova.df[1,5] <- resettest(modAfr)$p.value
valAncova.df[1,6] <- harvtest(modAfr)$p.value

yAme <- dataAncova[AncovaReg=="Americas",]$AncovaNew
xAme <- dataAncova[AncovaReg=="Americas",]$AncovaPop
modAme <- lm(yAme ~xAme)
summary(modAme)
plot(modAme)
valAncova.df[2,2] <- shapiro.test(rstandard(modAme))$p.value
valAncova.df[2,3] <- bptest(modAme)$p.value
valAncova.df[2,4] <- hmctest(modAme)$p.value
valAncova.df[2,5] <- resettest(modAme)$p.value
valAncova.df[2,6] <- harvtest(modAme)$p.value

yEas <- dataAncova[AncovaReg=="Eastern Mediterranean",]$AncovaNew
xEas <- dataAncova[AncovaReg=="Eastern Mediterranean",]$AncovaPop
modEas <- lm(yEas ~xEas)
summary(modEas)
plot(modEas)
valAncova.df[3,2] <- shapiro.test(rstandard(modEas))$p.value
valAncova.df[3,3] <- bptest(modEas)$p.value
valAncova.df[3,4] <- hmctest(modEas)$p.value
valAncova.df[3,5] <- resettest(modEas)$p.value
valAncova.df[3,6] <- harvtest(modEas)$p.value

yEur <- dataAncova[AncovaReg=="Europe",]$AncovaNew
xEur <- dataAncova[AncovaReg=="Europe",]$AncovaPop
modEur <- lm(yEur ~xEur)
summary(modEur)
plot(modEur)
valAncova.df[4,2] <- shapiro.test(rstandard(modEur))$p.value
valAncova.df[4,3] <- bptest(modEur)$p.value
valAncova.df[4,4] <- hmctest(modEur)$p.value
valAncova.df[4,5] <- resettest(modEur)$p.value
valAncova.df[4,6] <- harvtest(modEur)$p.value

ySou <- dataAncova[AncovaReg=="South-East Asia",]$AncovaNew
xSou <- dataAncova[AncovaReg=="South-East Asia",]$AncovaPop
modSou <- lm(ySou ~xSou)
summary(modSou)
plot(modSou)
valAncova.df[5,2] <- shapiro.test(rstandard(modSou))$p.value
valAncova.df[5,3] <- bptest(modSou)$p.value
valAncova.df[5,4] <- hmctest(modSou)$p.value
valAncova.df[5,5] <- resettest(modSou)$p.value
valAncova.df[5,6] <- harvtest(modSou)$p.value

yWes <- dataAncova[AncovaReg=="Western Pacific",]$AncovaNew
xWes <- dataAncova[AncovaReg=="Western Pacific",]$AncovaPop
modWes <- lm(yWes ~xWes)
summary(modWes)
plot(modWes)
valAncova.df[6,2] <- shapiro.test(rstandard(modWes))$p.value
valAncova.df[6,3] <- bptest(modWes)$p.value
valAncova.df[6,4] <- hmctest(modWes)$p.value
